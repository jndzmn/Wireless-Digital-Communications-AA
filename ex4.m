...
% noisy_bitstream = awgn(double(rs_encoded_bitstream.x),n,'measured'); 
% decoded_bitstream = rsdec(gf(uint32(noisy_bitstream), 8), 255, size(bitstream,2));
% failing_bits = xor(bitstream, decoded_bitstream.x);
